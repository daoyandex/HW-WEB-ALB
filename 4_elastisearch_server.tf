...
resource "yandex_compute_instance" "elastisearch-server" {
    zone = "ru-central1-a"
    name     = "elastisearch-server"
    hostname = "elastisearch-server"
    ...
    network_interface {
        ...
        security_group_ids = [yandex_vpc_security_group.elastisearch-sg.id]
